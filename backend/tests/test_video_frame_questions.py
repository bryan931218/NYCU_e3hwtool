...
class VideoFrameQuestionTests(unittest.TestCase):
    ...
    def test_renaming_marker_generates_summary_from_nearby_frames(self):
        ...
        self.assertIn("不設字數或行數限制", content[0]["text"])
        self.assertIn("重點｜", content[0]["text"])
        self.assertIn("關鍵｜", content[0]["text"])
        self.assertNotIn("120 個中文字內", content[0]["text"])
        self.assertNotIn("max_output_tokens", post.call_args_list[1].kwargs["json"])
    ...
    def test_marker_summary_is_not_truncated(self):
        marker = self.storage.create_study_plan_video_marker(
            video_id=self.video["id"],
            playback_seconds=45,
            note="完整長摘要",
        )
        long_summary = "這是一段需要完整保留的關鍵點摘要。" * 300

        saved = self.storage.update_study_plan_video_marker_summary(
            marker["id"],
            summary=long_summary,
            status="ready",
        )

        self.assertEqual(saved["summary"], long_summary)
        self.assertGreater(len(saved["summary"]), 2400)
    ...
    def test_page_renders_desktop_mobile_and_fullscreen_controls(self):
        ...
        self.assertIn("let frameQuestionHasResult = false", html)
        self.assertIn("if (!frameQuestionHasResult && !frameQuestionSubmitting)", html)
        self.assertIn("if (frameQuestionHasResult)", html)
        self.assertIn("frameQuestionHasResult = true", html)
        self.assertEqual(html.count("frameQuestionContext = null;"), 1)
        self.assertNotIn("frameQuestionRequestId += 1;", html)
        self.assertIn('id="frame-question-cancel" type="button">關閉</button>', html)
